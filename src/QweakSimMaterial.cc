...
G4cout << G4endl << "###### Calling QweakSimMaterial::DefineMaterials() " << G4endl << G4endl;
  ...
  Z      = 24.;
  A      = 52.00*g/mole;
  G4Element* elCr = new G4Element(name, symbol, Z, A);
  //---------------------------------------------------------
  name   = "Element_Magnesium";
  symbol = "Mn"; 
  Z      = 25.;
  A      = 54.94*g/mole;
  G4Element* elMn = new G4Element(name, symbol, Z, A);
  //---------------------------------------------------------
  name   = "Element_Iron";
  symbol = "Fe";
  Z      = 26.;
  ...
  Z      = 28.;
  A      = 58.70*g/mole;
  G4Element* elNi  = new G4Element(name, symbol, Z, A);
  //---------------------------------------------------------
  name   = "Element_Xenon";
  symbol = "Xe";
  Z      = 52.;
...
//	      matAl -> GetIonisation() -> SetMeanExcitationEnergy(166*eV);
 ...
 // photocathode material, approximated as elemental cesium
  ...
  density     = 8.02 * g/cm3;
  ncomponents = 5;
  G4Material* matStainlessSteel = new G4Material(name,density,ncomponents);
